...
# Paths assuming running from the root of the repository
# Using ONNX model for piece detection (has proper class names)
MODEL_PATH = "src/runs/detect_pieces_1.onnx"
BOARD_MODEL_PATH = "src/runs/chessboard_detection.pt"

try:
    model = YOLO(MODEL_PATH)
    board_model = YOLO(BOARD_MODEL_PATH)
except Exception as e:
    print(f"Error loading models from src/runs: {e}")
    # Fallback for paths if running from src directory
    MODEL_PATH = "runs/detect_pieces_1.onnx"
    BOARD_MODEL_PATH = "runs/chessboard_detection.pt"
    model = YOLO(MODEL_PATH)
    board_model = YOLO(BOARD_MODEL_PATH)
...
@app.post("/detect_fen")
async def detect_fen(file: UploadFile):
    """Detect chess pieces and board from an image, return FEN notation."""
    try:
...
if __name__ == "__main__":
    print("🚀 Starting Chess Detection Server...")
